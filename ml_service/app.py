...
MODEL_CONFIGS = {
    "BestModel": {
        ...
        "input_size": (224, 224),
        "preprocess": "rescale",
        "benchmark_accuracy": 0.8925,
        "description": "ResNet-50 (Best Model) - Architecture de référence haute performance",
    },
    "EfficientNetB0": {
        "candidate_paths": [
        ...
        "benchmark_accuracy": 0.8200,
        "description": "EfficientNet-B0 - Architecture légère optimisée",
    },
    "EfficientNetB3": {
        "candidate_paths": [
        ...
        "input_size": (300, 300),
        "preprocess": "efficientnet",
        "benchmark_accuracy": 0.7800,
        "description": "EfficientNet-B3 - Variante profonde pour analyse fine",
    },
    "AlzheimerCNN": {
        "candidate_paths": [
        ...
        "input_size": (224, 224),
        "preprocess": "rescale",
        "benchmark_accuracy": 0.7500,
        "description": "AlzheimerCNN - Modèle convolutionnel spécialisé IRM",
    },
    "EfficientNetB4": {
        "candidate_paths": [
        ...
        "input_size": (380, 380),
        "preprocess": "rescale",
        "benchmark_accuracy": 0.2500,
        "description": "EfficientNet-B4 - Modèle expérimental (Basse précision)",
    },
    "UltraBestModel": {
        "candidate_paths": [
        ...
        "input_size": (224, 224),
        "preprocess": "rescale",
        "benchmark_accuracy": 0.8500,
        "description": "UltraBestModel - Optimisation multi-architecture",
    },
}
...
def run_model(model_name: str, image_bytes: bytes) -> dict:
    t0 = time.time()
    mode = "real"
    benchmark_acc = MODEL_CONFIGS[model_name].get("benchmark_accuracy", 0.5)
    ...
    # Calcul du Score de Fiabilité Pondéré (RS)
    reliability_score = round(confidence * benchmark_acc, 4)
...
def select_best_model(results: dict) -> tuple[str, dict, list[str]]:
    """
    Sélection du modèle final basée sur le Score de Fiabilité Pondéré :
    Score = Confiance Actuelle * Précision Historique du Modèle
    Cela évite qu'un modèle peu fiable ne l'emporte avec une haute confiance erronée.
    """
    ...
    # Tri par Reliability Score décroissant
    ranked = sorted(candidate_names, key=lambda name: results[name]["reliability_score"], reverse=True)
    best_name = ranked[0]
...
def predict():
    ...
    return jsonify({
        ...
        "explanation": (
            f"Analyse multi-modèle complétée ({total_ms}ms). "
            f"Le verdict final est basé sur le Score de Fiabilité Pondéré (Confidence × Benchmark Accuracy). "
            f"Le modèle '{best_model_name}' a été retenu avec un score de {best['reliability_score']} "
            f"({best['confidence']*100:.1f}% de confiance × {best['benchmark_accuracy']*100:.1f}% de précision historique)."
        ),
        "all_models": results,
...
if __name__ == "__main__":
    print("\nDataScoop - Service ML demarrage")
    print("=" * 50)
    load_all_models()
    print("=" * 50)
